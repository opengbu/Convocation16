...
';
        ...
        mail($email, "Welcome To GBU Convocation", $message, $headers);
    }
    ...
    function get_roll_no() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('rollno_form');
        } else {
            $roll_number = html_purify($this->input->post('roll_number'));
            $query = $this->db->query("select * from studentinfo where enrollmentNo = '$roll_number'");
            $data_list = $query->row();
            ...
            if($data_list == null)
            {
                $error_msg = "Sorry Your Roll Number could not be found in our database. (Try in XX<b>/</b>XXX<b>/</b>XXX format).<br /> ";
                $error_msg .= "If you are still unable to register please email at exams@gbu.ac.in <br />";
                $error_msg .= "<a href = '". base_url('/register/get_roll_no') ."'>Retry</a>";
                $error_data = array(
                    'errors' => $error_msg,
                );

                $this->load->view('Error_message',$error_data);
                return;
            }


            $form_data = array(
                'full_name' => $data_list->studentName,
                'fathersName' => $data_list->fathersName,
                'year' => $data_list->year,
                'AreaSpecialization' => $data_list->AreaSpecialization,
                'degreeName' => $data_list->degreeName,
                'roll_number' => html_purify($this->input->post('roll_number')),
            );
            $this->load->view('registration_form', $form_data);
        }
    }
